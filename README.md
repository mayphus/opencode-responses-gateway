...
A narrow, stateless TypeScript gateway that accepts `POST /v1/responses` (or `/responses`), translates text/messages/function tools to OpenAI-compatible chat completions, and translates JSON or SSE results back to Responses objects/events.
...
- Text and function tools only; images are explicitly rejected instead of silently discarded. Files, audio, reasoning summaries, hosted tools, and structured-output translation are not supported.
- Stateless: `previous_response_id` is rejected. Clients must send full conversation input, including prior `function_call` and `function_call_output` items.
