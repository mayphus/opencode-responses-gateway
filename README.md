...
- Text, function tools, and free-form custom tools such as Codex `apply_patch` are supported. Custom tools are represented upstream as functions with one string `input`, then restored to native `custom_tool_call` items and streaming events. The gateway does not rewrite or repair tool input.
